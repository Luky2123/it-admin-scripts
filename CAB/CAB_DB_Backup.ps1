...
function Start-PrinterUpdate {
    ...
    Write-Host "  Prod. code + TL   - v20251030" -ForegroundColor Red
    ...
    # Logovanie
    # ----------------------------------------------
    $LogDir = "\\sk-rds-01\Powershell\CAB\Log_CAB_DB"
    ...
    # ----------------------------------------------
    # Zobrazenie zoznamu bez statusu
    # ----------------------------------------------
    ...
    function Update-Printer {
        ...
        try {
            if ($Number -le 5) {
                $localFolder = "\\sk-rds-01\CAB_DB\Prod.Code a user\L"
            } else {
                $localFolder = "\\sk-rds-01\CAB_DB\Prod.Code a user\P"
            }

            $ftpServer = "ftp://${IPAddress}/misc/"
            $user = "ftpcard"
            $password = "card"
